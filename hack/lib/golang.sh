...
kubeedge::golang::get_mappers_test_dirs() {
  (
    local findDirs
    local -a dirArray
    cd ${KUBEEDGE_ROOT}
    findDirs=$(find -L ./mappers  -name '*_test.go' -print0 | xargs -0n1 dirname | LC_ALL=C sort -u)
    dirArray=(${findDirs// /})
    echo "${dirArray[@]}"
  )
}
...
kubeedge::golang::run_test() {
  echo "running tests cases of mappers"

  cd ${KUBEEDGE_ROOT}
  
  testdirs=("$(kubeedge::golang::get_mappers_test_dirs)")
