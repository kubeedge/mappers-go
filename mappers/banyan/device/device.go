...
import (
	...
	"github.com/kubeedge/mappers-go/mappers/banyan/configmap"
	"github.com/kubeedge/mappers-go/mappers/banyan/driver"
	"github.com/kubeedge/mappers-go/mappers/banyan/globals"
	"github.com/kubeedge/mappers-go/mappers/common"
)

var devices map[string]*globals.BanyanDev
...
func setVisitor(visitorConfig *configmap.BanyanVisitorConfig, twin *common.Twin, client *driver.BanyanClient) {
...
func onMessage(client mqtt.Client, message mqtt.Message) {
	...
	for twinName, twinValue := range delta.Delta {
		...
		var visitorConfig configmap.BanyanVisitorConfig
		...
		setVisitor(&visitorConfig, &dev.Instance.Twins[i], dev.BanyanClient)
	}
}

// initBanyan initialize Banyan client.
func initBanyan(protocolConfig configmap.BanyanProtocolCommonConfig) (client *driver.BanyanClient, err error) {
	// TODO: pass the configuration to the client.
	config := driver.BanyanConfig{}
	client, _ = driver.NewClient(config)
	return client, nil
}

// initTwin initialize the timer to get twin value.
func initTwin(dev *globals.BanyanDev) {
	for i := 0; i < len(dev.Instance.Twins); i++ {
		var visitorConfig configmap.BanyanVisitorConfig
		if err := json.Unmarshal([]byte(dev.Instance.Twins[i].PVisitor.VisitorConfig), &visitorConfig); err != nil {
			klog.Errorf("Unmarshal VisitorConfig error: %v", err)
		...
		twinData := TwinData{Client: dev.BanyanClient,
...
func initData(dev *globals.BanyanDev) {
...
func initGetStatus(dev *globals.BanyanDev) {
	getStatus := GetStatus{Client: dev.BanyanClient,
...
func start(dev *globals.BanyanDev) {
	var protocolCommConfig configmap.BanyanProtocolCommonConfig
	...
	var protocolConfig configmap.BanyanProtocolConfig
	...
	client, err := initBanyan(protocolCommConfig)
	if err != nil {
		klog.Errorf("Init error: %v", err)
		return
	}
	dev.BanyanClient = client
...
func DevInit(configmapPath string) error {
	devices = make(map[string]*globals.BanyanDev)
