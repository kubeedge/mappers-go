...
const (
	...
	makeFilePath          = "../../../../mappers/bluetooth-go/"
	sourceConfigPath      = "./configuration/config.yaml"
	destinationConfigPath = "../../../../mappers/bluetooth-go/configuration/config.yaml"
	deployPath            = "../../../../mappers/bluetooth-go/deployment.yaml"
)
...
func TestMapperCharacteristics(t *testing.T) {
	...
	var _ = BeforeSuite(func() {
		...
		Expect(err).Should(BeNil())
		cmd = exec.Command("make", "bluetooth-go_image")
		err = utils.PrintCombinedOutput(cmd)
		Expect(err).Should(BeNil())

		//dockertag
		tagname := cfg.DockerHubUserName + "/bluetooth-go:v1.0"
		cmd = exec.Command("docker", "tag", "bluetooth-go:v1.0", tagname)
		err = utils.PrintCombinedOutput(cmd)
		Expect(err).Should(BeNil())
